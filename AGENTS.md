...
A pre-commit hook (`.githooks/pre-commit`, activated by the `prepare` script via `core.hooksPath`) runs the test suite and blocks the commit on any failure. Bypass with `git commit --no-verify` only when necessary.
